...
const AuthModal = () => {
  ...
  // Local state to track the current action (sign in or sign up)
  const [action, setAction] = useState(actionState.signin);

  // Effect to reset the action to signin whenever the modal is opened
  useEffect(() => {
    ...
    <Modal open={authModalOpen} onClose={handleClose}>
      <Box sx={{
        ...
        <Box sx={{ padding: 4, boxShadow: 24, backgroundColor: "background.paper" }}>
          ...
          {/* Conditionally render the SigninForm or SignupForm based on the current action state */}
